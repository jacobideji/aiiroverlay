...
| 55–60 | **Convene the [Materiality and Disclosure call](../framework/04-materiality-and-disclosure.md)** if any of the canonical convening triggers from framework/04 applies (Mode M3 or higher activated; or any condition-based trigger including external recipients, regulated data, customer-facing trust impact, or financial actions). For workflow-injection incidents specifically, external recipients of agent output and touched regulated data are the most common condition-based triggers. The convening protocol applies regardless of whether the agent's action was technically reversible. | Incident Commander |
