...
— then the rest of the framework's machinery (Inventory, Safe Modes, Evidence Set, Controlled Re-Enable) snaps into place. If your team cannot, you will respond to an AI incident as if it were a software bug or a model-behavior question, and you will destroy evidence and over- or under-contain before you understand the blast radius.

**Use this playbook when:** an AI agent with tool access exhibits unintended behavior, an external researcher reports suspected prompt injection, retrieval traces show suspicious sources, or a downstream system (CRM, email, ticketing) shows actions the business did not authorize.
...
Walk the [Six Triage Questions](../triage/six-questions.md) **in order** on the initial bridge call. Do not skip ahead — each question scopes the next.
...
**Discipline:** if any answer takes more than 60 seconds to produce, you have an *inventory problem*, not an incident problem — and the inventory gap is itself a finding. Document it in the decision log; do not slow the bridge call to fix it.
...
Rotating tokens before capturing scopes is the single most common evidence-destruction failure in AI IR. It is the reason this playbook lists "evidence plan before key rotation" as Triage Question 6.
...
**Operational requirement:** the team must be able to **export the full set within 60 minutes** of incident declaration. If you cannot, you do not have a Level 3 (Provable) [Maturity Roadmap](../framework/03-maturity-roadmap.md) capability — log the gap.
...
| **Post-incident comms** | Prepare the recovery communication per [`playbook-18`](../crosswalks/nist-csf-2.md) (Post-Incident Hardening) — to internal stakeholders, customers, regulators if scope warrants. |
...
|---|---|---|
| **Rotating tokens before capturing scopes** | "Stop the bleeding" reflex from traditional IR | Cannot prove what the agent could do — scope becomes a guess |
| **Updating the system prompt before exporting type E** | Owner wants to "fix it now" | Cannot prove what the agent was told — defensibility collapses |
| **Cleaning the knowledge base before preserving versioned content** | Operations team treats corpus as logs | Cannot prove the input vector for context-poisoning class incidents |
