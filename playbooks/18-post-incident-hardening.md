...
**SLA realism (table-stakes vs stretch):** the 5-business-day target is achievable for organizations with a streamlined change-control board, an empowered Incident Commander, and engineering capacity reserved for post-incident work. For organizations whose normal change-control cycle is 2-to-4 weeks (large-enterprise CAB review, security architecture review, deployment-window constraints), 5 business days is a stretch target rather than a table-stakes commitment. Adopters in this profile should either expedite a fast-track CAB path for post-incident hardening (the recommended discipline) or document the customer's realistic timeline (e.g., 10 business days) and queue CAB acceleration as a backlog item. The framework's 5-day target is the discipline goal; conformance still holds at the customer's realistic timeline provided the gap is documented and being closed.
