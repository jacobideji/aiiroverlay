...
> *Good post-incident hardening does not slow the business — it prevents the repeat failure that would. The measure is simple: would a recurrence of the triggering prompt be contained by the controls you shipped after the incident?*

## Premise

Post-incident analysis is routine in cybersecurity and IT, but AI incidents bring a particular failure mode: most organizations close them out with **prompt updates, user reminders, or vendor notifications** — interventions that feel decisive in the moment but address symptoms rather than systemic risk. Prompts get rewritten. The next prompt-injection variant bypasses the rewrite. The same incident recurs three months later with a slightly different vector.
...
| 10–25 | List every candidate control discovered during the incident — including ones that *would not* have prevented this incident but *should* exist anyway. Do not filter yet. |
...
That is the entire first hour. The fix list at the end of it should fit on one page. If it does not, you are confusing *hardening priorities* with a *backlog grooming session* — separate the two.

**Critical:** every selected control must be **measurable**. *"Tighten the email tool"* is not a hardening control — it is a wish. *"Convert email tool from `send` to `draft` for any external recipient; CI rejects calls to `send.external` without two-person approval"* is a hardening control. Specificity is what makes the 5-day SLA enforceable.
...
|---|---|
| **M1 Read-Only** | Pre-defined `read_write` flag on every tool ([Privilege Matrix](../templates/agent-privilege-matrix.csv)) → M1 is a config flip, not a code change |
| **M2 Approvals Required** | Approval-gate infrastructure already wired for Tier 2 tools → M2 is enabling a flag, not building a workflow under pressure |
| **M3 Tool Tiering** | Every tool already tiered per [Playbook 04](04-tool-design-is-containment.md) → M3 is a CSV filter, not a forensic exercise |
...
**Operational requirement (post-hardening):** the next incident's evidence export must complete within **45 minutes** — 25% faster than the framework's 60-minute baseline. The hardening dividend is measurable speedup, not just better documentation.
...
- **Sensitive corpora isolation by role:** policy documents, security playbooks, financial procedures should never be retrieved by agents whose business purpose does not require them.
...
Hardening is not a separate phase from recovery — it is the **validation gate** between Mode M5 and the return to M0 Observe. An agent that re-enters production without hardening has not actually completed recovery; it has only resumed operations under the same conditions that produced the incident.

The recovery sequence with hardening built in:

1. **Re-enable in Read-Only (M1)** — confirm the agent functions; logs flow.
2. **Validate retrieval and tool policies** — *and* deploy the first hardening fix (typically a Tier 2 tool downgrade or allowlist tightening).
3. **Replay the incident scenario in a safe harness** — *and* verify the new hardening control would have prevented it. **If it would not, the hardening is insufficient.**
4. **Re-enable tools incrementally** — *and* monitor the new hardening control under live traffic for the first 24–72 hours.
...
The replay step (3) is the hardening **acceptance test**. If a recurrence of the triggering prompt would not be contained by the new control, the recovery is not complete — go back to the fix list.
...
- **Export procedures are tested regularly** — the first time you discover your export procedure has friction should not be during an active incident.
...
These can be useful in their own right, but they are **not the 5-day SLA deliverable**. The SLA deliverable is a code, config, or policy change with measurable risk reduction.
...
| **No metric tracking** | Hardening seen as one-time | Cannot demonstrate trend improvement; [Maturity Roadmap](../framework/03-maturity-roadmap.md) Level 4 (Resilient) unreachable |
...
If no, the hardening is theater — and the next recurrence is on the schedule, you just do not know which week.

That is the test. That is the discipline. That is what turns lessons learned into permanent guardrails.
