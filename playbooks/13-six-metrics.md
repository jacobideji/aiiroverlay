...
2. **The [Board-Ready Scorecard](24-board-ready-scorecard.md) inherits from the metrics.** Containment, Evidence, and Recovery domains map directly to Six Metric values: Containment = Metrics 2 and 4; Evidence = Metric 3; Recovery = Metrics 5 and 6. The Governance domain is structurally different: it tracks discipline signals (tier discipline at C1, M3 operational readiness at C2, materiality documentation at C3, risk-acceptance tracking at C4) that are not directly metric-quantified. These signals depend on the AI-BOM substrate whose currency is tracked by Metric 1 (inventory currency), but the C-rows themselves are scorecard-tracked rather than metric-tracked.
