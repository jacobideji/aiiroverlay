...
2. **The [Board-Ready Scorecard](24-board-ready-scorecard.md) inherits from the metrics.** The four scorecard domains (Containment, Evidence, Governance, Recovery) map to combinations of the Six Metrics. Containment domain = Metrics 2 and 4. Evidence domain = Metric 3. Governance domain = Metric 1 (inventory currency, for scorecard item C1) plus governance-discipline signals (tool tiering at C2, materiality documentation at C3, risk-acceptance tracking at C4) that are scorecard-tracked rather than metric-tracked. Recovery domain = Metrics 5 and 6.
