...
| 55–60 | **Convene the [Materiality and Disclosure call](../framework/04-materiality-and-disclosure.md)** if any of the canonical convening triggers from framework/04 applies. For Shadow AI discoveries specifically, the condition-based triggers most commonly applicable are customer data, regulated data, external recipients, financial actions, or customer-facing trust impact identified through the retrospective CIA+T assessment. Shadow agent discovery often surfaces materiality questions that have been latent for weeks or months; the convening protocol applies regardless of containment mode because the disclosure window may have started before the agent was discovered. | Incident Commander |
