...
| 55–60 | **Convene the [Materiality and Disclosure call](../framework/04-materiality-and-disclosure.md)** if any of the canonical convening triggers from framework/04 applies (Mode M3 or higher activated; or any condition-based trigger including external recipients, regulated data, customer-facing trust impact, or financial actions). Vendor-copilot incidents nearly always meet the canonical triggers because the vendor copilot operates against the customer's brand and identity, frequently reaching external recipients (customer messages, partner systems, public output) and regulated data classes. The convening protocol's customer-side track does not wait for vendor cooperation. | Incident Commander |
...
> **When contractual leverage is limited:** the Boundary 1 (Contract) hardening items assume meaningful customer negotiating power. Customers without that leverage (typically mid-market organizations on standard vendor terms, or any customer whose vendor renewal is months away) face a deployment decision the framework does not paper over. The playbook's defensive thesis treats Boundary 1 as a precondition for conformant deployment; a customer who chooses to deploy without it is making a **documented risk-acceptance decision, not claiming conformance to the framework's standard**. The risk acceptance is signed by the CISO, recorded in the customer's audit trail, and surfaces as scorecard item **C4** in the [Playbook 24 Board Scorecard](24-board-ready-scorecard.md) Governance domain (Domain C) until Boundary 1 closes at the next contract renewal cycle. In the interim, the customer leans harder on the items they control unilaterally: Boundary 2 (Identity) restricting vendor agent OAuth scopes at the customer's IdP, Boundary 3 (Evidence) extending customer-side audit-log retention beyond default, and Boundary 4 (Communication) hardening internal escalation paths. The quarterly Vendor Evidence Drill in this risk-accepted variant runs as an **internal-only drill** simulating vendor cooperation failures, until contractual provisions exist to drill against the real vendor.
