...
- **DE.AE-06:** *"Information on adverse events is provided to authorized staff and tools."* The stakeholder notification and operational alerting specified by [Playbook 11: Monitoring That Truly Detects Agent Incidents](../playbooks/11-monitoring-detection.md) (three signal families for authorized misuse, with a 60-second containment-action latency budget) feed this subcategory.
...
- **Mapping completeness:** all six CSF 2.0 functions (GOVERN, IDENTIFY, PROTECT, DETECT, RESPOND, RECOVER) have substantive playbook coverage. GV.SC (supply chain risk management) procurement-time discipline for AI platforms is covered by [Playbook 19: Build vs Buy for Agent Controls](../playbooks/19-build-vs-buy.md). ID.AM (asset management) operational discipline for AI agents, including the Shadow AI discovery boundary that closes the inventory-gap precondition, is covered by [Playbook 21: Shadow AI](../playbooks/21-shadow-ai.md). ID.RA (risk assessment of change events) and PR.PS (platform security configuration management) applied to AI agents are covered by [Playbook 22: Model and Policy Drift](../playbooks/22-model-policy-drift.md), with the change-pipeline event ledger and the Post-Change Configuration Snapshot as the empirical evidence that supports both subcategories. **PR.PS-06** (secure configuration of the logging pipeline) is covered by [Playbook 23: AI Logging and Privacy in a Multi-Stakeholder World](../playbooks/23-logging-privacy.md), with the Three-Layer Logging Model and the redaction-and-tokenization discipline as the secure-configuration artifacts. PR.AA-05 (access policies on AI agent identities) is covered by [Playbook 07: Secrets and Tokens](../playbooks/07-secrets-and-tokens.md); PR.AA extended to access control on the evidence store itself (role-separated access, audited break-glass procedures, explicit per-evidence-type authorization, quarterly access-audit review) is covered by [Playbook 23: AI Logging and Privacy in a Multi-Stakeholder World](../playbooks/23-logging-privacy.md). PR.DS-01 (data-at-rest protection for agent memory and retrieval corpora, for the evidence store itself, and for the privacy controls applied to AI logs that simultaneously serve as evidence and as regulated-data stores) is covered by [Playbook 03: RAG / Knowledge-Base Forensics](../playbooks/03-rag-knowledge-base-forensics.md) (corpus and vector-store integrity discipline), [Playbook 06: Rethinking Prompt Injection as a Workflow Threat](../playbooks/06-prompt-injection-workflow.md) (content-trust labeling at the ingestion boundary), [Playbook 15: Records, Retention, and Proving What Happened](../playbooks/15-records-retention.md) (the evidence store as a Tier-T2 asset with chain-of-custody discipline, tamper-evident integrity, and the Two-Tier Retention Standard), and [Playbook 23: AI Logging and Privacy in a Multi-Stakeholder World](../playbooks/23-logging-privacy.md) (the Three-Layer Logging Model, the redaction-and-tokenization discipline, and the multi-stakeholder governance discipline that calibrate AI log content so the privacy posture and the forensic posture are simultaneously defensible). PR.DS-02 (data-in-transit protection on the output path, including output-layer DLP, channel classification, and destination-aware approval gates) is covered by [Playbook 09: Leakage Without a Breach](../playbooks/09-output-leakage.md). DE.CM (continuous monitoring), including **DE.CM-01** (networks and network services monitored for adverse events) and **DE.CM-09** (computing hardware, software, runtime environments, and their data monitored for adverse events), is covered by [Playbook 11: Monitoring That Truly Detects Agent Incidents](../playbooks/11-monitoring-detection.md) and extended by [Playbook 22: Model and Policy Drift](../playbooks/22-model-policy-drift.md) along the drift-detection dimension (tool-invocation-frequency, retrieval-pattern, and refusal-pattern signals that surface change-event regressions independent of any specific external trigger). **RS.AN-06** (actions performed during an investigation are recorded; records' integrity and provenance are preserved) and **RS.AN-07** (incident data and metadata are collected; their integrity and provenance are preserved) are covered by [Playbook 15: Records, Retention, and Proving What Happened](../playbooks/15-records-retention.md), with the chain-of-custody discipline, the tamper-evidence anchor, the integrity manifest, and the quarterly Reconstructability Test as the empirical artifacts that support both subcategories. **RS.CO** (incident communication during response) and **RC.CO** (recovery communication) are covered by [Playbook 17: Communication Techniques for AI-Involved IR](../playbooks/17-communication-techniques.md), with the 30-minute first-update SLA, the Three-Status Taxonomy (Confirmed, Suspected, Validating), the Four-Element Update Standard (factual impact, immediate containment, evidence-collection activity, next-update timing), the Stakeholder Communication Matrix (eight stakeholder classes each with calibrated cadence and content scope), the Template Library, and the Responsible Reframing discipline as the empirical artifacts. GV.OC (organizational context including stakeholder communication) extended to include PB17's Stakeholder Communication Matrix as the operational specification for AI-incident stakeholder communication. **PR.AT-01** (general awareness and training) and **PR.AT-02** (personnel performing specialized cybersecurity roles) are covered by [Playbook 16: Training Your Team for AI Incidents](../playbooks/16-training-your-team.md), with the 30-Minute Micro-Drill (Trigger and Contain → Pull Evidence → Scope and Brief, each 10 minutes), the Four Core Moves, the Curriculum-of-Six (safe modes, tool tiering, retrieval traces, tool-call logs, memory state, configuration snapshots), and the monthly drill cadence as the empirical artifacts. **GV.RR** extended to include the Safe Mode Owner and Evidence Owner role definitions from PB16 as the operational specification for AI IR roles, responsibilities, and authorities. **GV.OV** (organizational oversight by senior leaders), **GV.OC** (organizational context for executive decision-making during AI incidents), and **GV.RM** (organizational risk-management decision-making) are covered by [Playbook 05: Executive Decision-Making With AI in the Loop](../playbooks/05-executive-decision-making.md), with the Executive Decision Packet (AI Edition) five-section structured update, the CIA+T impact framing that elevates Trust to peer status alongside Confidentiality, Integrity, and Availability, the 4-hour cadence, the 4/24/72-hour planning horizons, and the Approval Receipt discipline as the empirical artifacts.
- **Foundational and cross-cutting playbooks:** [Playbook 02: Evidence Lives in New Places](../playbooks/02-evidence-lives-in-new-places.md) establishes the foundational evidence-preservation reflexes (the Three Realities of AI Evidence: the actor is a workflow, not a workstation; the payload can be language, not malware; evidence is fragile) that underpin every DE.AE (adverse event analysis) and RS.AN (response analysis) activity for AI agents. [Playbook 13: The Six Metrics](../playbooks/13-six-metrics.md) operationalizes ID.IM (improvements identified across all CSF functions) with six measurable quantities (Inventory Currency, Containment Time-to-Activate, Evidence Export Time, Drill Currency, Hardening SLA Compliance, Controlled Re-Enable Success Rate) that feed the PB24 board scorecard. [Playbook 14: Testing for Agent Failure Modes](../playbooks/14-testing-for-agent-failure-modes.md) operationalizes ID.RA (risk assessment) for AI agents through pre-production failure-mode testing of the kill-switch ladder and forms the system-side half of the testing-and-training pair with [Playbook 16](../playbooks/16-training-your-team.md). [Playbook 20: AI IR Maturity Roadmap (operating view)](../playbooks/20-maturity-roadmap.md) operationalizes GV.RM (organizational risk management strategy) through the four maturity levels (Aware, Containable, Provable, Resilient) and the conformance criteria for each.
