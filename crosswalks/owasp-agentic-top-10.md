...
- **Mapping completeness:** all 10 ASI categories have substantive playbook coverage.
  ...
  - **ASI02 Tool Misuse & Exploitation** and **ASI05 Unexpected Code Execution:** [Playbook 04](../playbooks/04-tool-design-is-containment.md) (prevention-side via tool tiering T0/T1/T2). Response-side composition for active code-execution events: [Playbook 01](../playbooks/01-agent-as-privileged-identity.md) (privileged-identity response) plus [M4 Full Disable](../kill-switches/overview.md) (active-harm containment) plus [Playbook 09](../playbooks/09-output-leakage.md) for the output distribution map (Type F evidence for any data exfiltrated by the executed code). A dedicated PB25 response variant for sandbox-escape and RCE-via-tool incidents is a v1.1 candidate.
  - **ASI03 Identity & Privilege Abuse:** [Playbook 07](../playbooks/07-secrets-and-tokens.md) (credential discipline) and [Playbook 21](../playbooks/21-shadow-ai.md) (discovery boundary that closes the inventory-gap precondition: shadow agents operate with credentials that ASI03 detection cannot evaluate if the agent is not in the inventory).
  - **ASI04 Agentic Supply Chain Compromise:** [Playbook 07](../playbooks/07-secrets-and-tokens.md) (vendor credential discipline), [Playbook 10](../playbooks/10-vendor-copilots.md) (the framework's dedicated vendor-copilot response playbook), [Playbook 19](../playbooks/19-build-vs-buy.md) (the procurement-time discipline that closes the precondition gap by selecting platforms that can support ASI04 response in the first place), and [Playbook 21](../playbooks/21-shadow-ai.md) (vendor-hosted shadow agents are an ASI04 surface that the discovery boundary addresses before they cause an incident).
  - **ASI06 Memory & Context Poisoning:** Primary response: [Playbook 03](../playbooks/03-rag-knowledge-base-forensics.md) (RAG forensics), [Playbook 06](../playbooks/06-prompt-injection-workflow.md) (workflow-injection vector), [Playbook 11](../playbooks/11-monitoring-detection.md) (detection), and [Playbook 22](../playbooks/22-model-policy-drift.md) (change-event forensics when memory or retrieval configuration changes accumulate as drift). Secondary coverage: [Playbook 09](../playbooks/09-output-leakage.md) (output-side response when memory carries leaked content forward) and [Playbook 23](../playbooks/23-logging-privacy.md) (memory snapshots as payload-class evidence; the redaction-and-tokenization discipline applies).
  ...
  - **ASI09 Human-Agent Trust Exploitation:** [Playbook 24](../playbooks/24-board-ready-scorecard.md) (governance-side trust posture), [Playbook 17](../playbooks/17-communication-techniques.md) (incident-time trust-preservation discipline: when AI trust is broken by an incident, the 30-minute first-update SLA, the Three-Status Taxonomy, the Stakeholder Communication Matrix, the Template Library, and the Responsible Reframing discipline are the trust-rebuilding machinery that runs alongside the framework's technical response), [Playbook 16](../playbooks/16-training-your-team.md) (the operator-training discipline that makes the Mental Model's accountability framing and the framework's response disciplines executable under pressure: the 30-Minute Micro-Drill, the Four Core Moves, the two permanent roles, the Curriculum-of-Six, and the monthly cadence with measurable training targets), and [Playbook 05](../playbooks/05-executive-decision-making.md) (the executive-decision-making discipline that operationalizes accountable decisions under uncertainty when AI trust is broken: the Executive Decision Packet AI Edition with five sections including the Agent Capability Profile and Provenance Summary, the CIA+T impact framing that elevates Trust to peer status, the 4-hour cadence, the 4/24/72-hour planning horizons, and the Approval Receipt discipline that prevents human approval from degrading into rubber-stamping). **Coverage qualification:** the framework's ASI09 coverage is governance-side, response-side, and training-side rather than detection-side. Detection of trust miscalibration (user overreliance on incorrect AI output) and trust-calibration drills are v1.1 candidates.
  - **ASI10 Rogue Agents:** [Playbook 11](../playbooks/11-monitoring-detection.md) (detection signals), [Playbook 12](../playbooks/12-insider-threat-3.md) (response and investigation; the matched detection-response pair completing the rogue-agent coverage arc), and [Playbook 22](../playbooks/22-model-policy-drift.md) (change-event forensics for the dominant 2026 form of rogue-agent emergence: unintentional drift accumulation through model upgrades, prompt edits, policy tunes, retriever changes, and index rebuilds rather than reward hacking or goal collusion).
- **Coverage status:** complete through `v0.24.0` (content gate complete; all 24 playbooks shipped). The two largest ASI gaps from the v0.10.x baseline are closed: ASI04 supply-chain response by [Playbook 10](../playbooks/10-vendor-copilots.md) in `v0.13.0`, and ASI10 rogue-agent response by [Playbook 12](../playbooks/12-insider-threat-3.md) in `v0.11.0`. The OWASP Top 10 for LLM Applications 2025.1 **LLM02 Sensitive Information Disclosure** is operationalized by [Playbook 09: Leakage Without a Breach](../playbooks/09-output-leakage.md) in `v0.15.0` on the output side; [Playbook 23: AI Logging and Privacy in a Multi-Stakeholder World](../playbooks/23-logging-privacy.md) in `v0.20.0` extends LLM02 coverage to the evidence side, addressing the case where the logs that exist to support the LLM02 response are themselves disciplined so they do not become an LLM02 finding (the Three-Layer Logging Model, the redaction-and-tokenization discipline, and the multi-stakeholder governance discipline are the operational artifacts). The **inventory-gap precondition** that made ASI03 (Identity & Privilege Abuse) and ASI04 (Agentic Supply Chain Compromise) harder to detect against shadow agents is closed by [Playbook 21: Shadow AI](../playbooks/21-shadow-ai.md) in `v0.16.0`. The **procurement-time precondition** that determines whether a platform can support ASI04 response at all is closed by [Playbook 19: Build vs Buy for Agent Controls](../playbooks/19-build-vs-buy.md) in `v0.17.0`. The **change-event precondition** that determines whether ASI06 and ASI10 manifestations can be traced to specific deployment events (rather than misdiagnosed as external attacks or dismissed as routine production variation) is closed by [Playbook 22: Model and Policy Drift](../playbooks/22-model-policy-drift.md) in `v0.18.0`, with the Post-Change Configuration Snapshot, the change-pipeline event ledger, the Drift Canary pack, and the layered rollback sequence as the empirical artifacts. The **proof-discipline precondition** that determines whether the response to any ASI category can be defended in a regulator, legal, or board review weeks or months later is closed by [Playbook 15: Records, Retention, and Proving What Happened](../playbooks/15-records-retention.md) in `v0.19.0`. PB15 is structurally cross-cutting across all 10 ASI categories: without defensible evidence retention, the response disciplines for ASI01 through ASI10 produce findings that cannot survive the regulatory, legal, or business-trust review window. PB15 contributes the Two-Tier Retention Standard, the incident-triggered legal-hold mechanism, the chain-of-custody discipline, the tamper-evidence anchor, and the quarterly Reconstructability Test as the proof-side machinery the framework's response claims depend on. The **privacy-discipline precondition** that determines whether the framework's evidence-capture posture is defensible by all four stakeholder groups (Security, Privacy, Legal, Engineering) is closed by [Playbook 23: AI Logging and Privacy in a Multi-Stakeholder World](../playbooks/23-logging-privacy.md) in `v0.20.0`. PB23 is also structurally cross-cutting: without the multi-stakeholder governance discipline, the framework's evidence claims for any ASI category produce either overcollection findings under data-minimization regulations or undercollection findings under the framework's own Reconstructability Test. PB23 contributes the Multi-Stakeholder Governance Matrix, the Three-Layer Logging Model, the Forensically Useful standard, and the redaction-and-tokenization discipline as the capture-side machinery that makes the framework's response claims simultaneously privacy-defensible and forensically defensible. The **communication-discipline precondition** that determines whether the customer's response to any ASI category can preserve stakeholder trust through the response window is closed by [Playbook 17: Communication Techniques for AI-Involved IR](../playbooks/17-communication-techniques.md) in `v0.21.0`. PB17 is also structurally cross-cutting: without disciplined communication, the framework's technical response for any ASI category can produce poor stakeholder outcomes regardless of how well the technical containment performed. PB17 contributes the 30-minute first-update SLA, the Three-Status Taxonomy (Confirmed, Suspected, Validating), the Four-Element Update Standard, the Stakeholder Communication Matrix, the Template Library, and the Responsible Reframing discipline as the trust-preservation machinery that runs alongside the framework's technical response across all 10 ASI categories. The **training-discipline precondition** that determines whether the framework's response disciplines are executable under operational pressure across all 10 ASI categories is closed by [Playbook 16: Training Your Team for AI Incidents](../playbooks/16-training-your-team.md) in `v0.22.0`. PB16 is also structurally cross-cutting: without trained execution, the framework's documented disciplines for any ASI category produce poor outcomes regardless of how technically sound the documentation is. PB16 contributes the 30-Minute Micro-Drill, the Four Core Moves, the two permanent operating roles (Safe Mode Owner and Evidence Owner), the Curriculum-of-Six, the monthly cadence, and the measurable training targets as the human-side machinery that makes the framework's response claims operationally executable. The **executive-decision-making discipline precondition** that determines whether the customer's executive layer can make defensible decisions under uncertainty across all 10 ASI categories is closed by [Playbook 05: Executive Decision-Making With AI in the Loop](../playbooks/05-executive-decision-making.md) in `v0.24.0`. PB05 is the content-gate-completing playbook; all 24 playbooks (PB01 through PB24) are now shipped. PB05 contributes the Executive Decision Packet (AI Edition), the CIA+T impact framing, the 4/24/72-hour planning horizons, the Approval Receipt discipline, and the Three Executive Routine Additions as the executive-layer machinery that converts the framework's response posture into board-defensible, regulator-defensible, customer-defensible decisions during AI incidents. Future framework releases focus on governance maturation (Steering Committee announcement, public-interface freeze, v1.0.0 cut) rather than additional playbook content.
