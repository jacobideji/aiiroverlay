...
- **Coverage status:** complete through `v0.15.0`. The two largest ASI gaps from the v0.10.x baseline are closed: ASI04 supply-chain response by [Playbook 10](../playbooks/10-vendor-copilots.md) in `v0.13.0`, and ASI10 rogue-agent response by [Playbook 12](../playbooks/12-insider-threat-3.md) in `v0.11.0`. The OWASP Top 10 for LLM Applications 2025.1 **LLM02 Sensitive Information Disclosure** is now operationalized by [Playbook 09: Leakage Without a Breach](../playbooks/09-output-leakage.md) in `v0.15.0`, closing the output-side gap that prior releases only addressed implicitly through PB03 (retrieval forensics) and PB06 (workflow injection input-side). Future playbooks will add ASI-cross-cutting controls and deepen coverage on emerging variants.
