# Content Map — Newsletter Issues → Repository Files

This repository's framework, playbooks, crosswalks, and templates trace directly to the *AI IR Overlay™* LinkedIn newsletter series authored by Jacob Ideji (2025–2026). Each newsletter issue maps to a specific home in this repo. This file is the canonical "where did this come from?" reference.

## How releases ship

The framework ships incrementally. Earlier plans assumed one playbook per release in newsletter order (PB01 → v0.2, PB02 → v0.3, …). The actual ship arc has been **narrative-driven** rather than sequential:

1. **Foundation** (`v0.1.x`) — framework core, triage, kill-switches, evidence, templates.
2. **Response** (`v0.2.0`) — Playbook 01 establishes the privileged-identity lens every later playbook builds on.
...
6. **Measurement & Depth** (`v0.6.0`) — Playbook 13 (Six Metrics) + Playbook 03 (RAG Forensics) + Playbook 14 (Testing) make the arc measurable and operationally deep.

The remaining playbooks ship as future MINOR releases. `v1.0.0` is cut as the stable release once all 24 playbooks are live and a Steering Committee is announced (see [GOVERNANCE.md](GOVERNANCE.md)).
...
| 3 | RAG / Knowledge-Base Forensics | [`playbooks/03-rag-knowledge-base-forensics.md`](playbooks/03-rag-knowledge-base-forensics.md) | ✅ `v0.6.0` |
...
| 20 | AI IR Maturity Roadmap | [`framework/03-maturity-roadmap.md`](framework/03-maturity-roadmap.md) (framework view) · `playbooks/20-maturity-roadmap.md` (operating view) | ✅ framework view (`v0.1.0`) · 🟡 operating view drafted |
...
| 23 | AI Logging and Privacy in a Multi-Stakeholder World | `playbooks/23-logging-privacy.md` · [`crosswalks/nist-ai-rmf.md`](crosswalks/nist-ai-rmf.md) | ✅ crosswalk (`v0.1.5`) · 🟡 playbook drafted |
| 24 | Board-Ready Scorecard | [`playbooks/24-board-ready-scorecard.md`](playbooks/24-board-ready-scorecard.md) · `templates/board-scorecard.md` (printable) | ✅ playbook (`v0.5.0`) · ⬜ printable template planned |
...
| Board Scorecard (printable) | `templates/board-scorecard.md` | ⬜ planned (ships with PB24 operating revision) |

## Drafted-but-unshipped: conversion notes

Each playbook marked **🟡 drafted** has source content in the maintainer's working folder (`AI IR Overlay-Linkedin-Post_completed/` and the package drafts in `aiiroverlay-packages/`). Promoting a drafted playbook to a shipped release requires expanding the draft from its newsletter footprint (~7 KB) to a full operational playbook (~15–20 KB) following the established skeleton:

1. **Premise** — what's AI-specific (1–2 paragraphs)
2. **First-Hour Actions** — table with minute markers and owners
3. **Containment Options** — mapped to Kill-Switch Modes M0–M5
4. **Evidence Priorities** — which of the A–F evidence types are load-bearing for this scenario
5. **Recovery Sequence** — staged re-enablement, MVO-4 alignment
6. **Post-Incident Hardening** — what enters [Playbook 18](playbooks/18-post-incident-hardening.md)'s 5-business-day SLA
7. **Common Pitfalls** — failure modes specific to this scenario class
8. **Related** — relative-path links to every framework artifact and adjacent playbook
9. **The Question to Carry Forward** — single-question close

The current live playbooks ([PB01](playbooks/01-agent-as-privileged-identity.md), [PB03](playbooks/03-rag-knowledge-base-forensics.md), [PB04](playbooks/04-tool-design-is-containment.md), [PB13](playbooks/13-six-metrics.md), [PB14](playbooks/14-testing-for-agent-failure-modes.md), [PB18](playbooks/18-post-incident-hardening.md), [PB24](playbooks/24-board-ready-scorecard.md)) are the structural reference for tone, depth, and cross-linking.

## Citation rule

Every playbook MUST include in its footer:
...
This preserves academic citability and supports the trademark and attribution model documented in [LICENSE](LICENSE).

## Why this file exists

A reader who lands on the framework with the question *"where are playbooks 02, 05–12, 15–17, 19–23?"* deserves an honest answer. This file is that answer: every issue has a destination, every shipped artifact has a release tag, and every drafted-but-unreleased artifact is named so it can't pretend to be absent. The framework's promise is incremental shipping. This map is the receipt.
